...
class GroupCallEventHandler {
      ...
      if (eventType === _event.EventType.GroupCallPrefix) {
        ...
        if (!currentGroupCall && !content["m.terminated"]) {
          ...
          if (content["m.terminated"]) {
            ...
            _logger.logger.warn(`The group call type changed for room: ${state.roomId}. Changing the group call type is currently unsupported.`);
          }
        } else if (currentGroupCall && currentGroupCall.groupCallId !== groupCallId) {
          // TODO: Handle new group calls and multiple group calls
          _logger.logger.warn(`Multiple group calls detected for room: ${state.roomId}. Multiple group calls are currently unsupported.`);
        }
      }
    });
  }
  async start() {
    ...
    if (this.client.getSyncState() !== _sync.SyncState.Syncing) {
      _logger.logger.debug("Waiting for client to start syncing...");
  ...
  createGroupCallForRoom(room) {
    ...
    for (const callEvent of sortedCallEvents) {
      ...
      _logger.logger.debug(`Choosing group call ${callEvent.getStateKey()} with TS ` + `${callEvent.getTs()} for room ${room.roomId} from ${callEvents.length} possible calls.`);
      this.createGroupCallFromRoomStateEvent(callEvent);
      break;
    }
    _logger.logger.info("Group call event handler processed room", room.roomId);
  ...
  createGroupCallFromRoomStateEvent(event) {
    ...
    if (!room) {
      _logger.logger.warn(`Couldn't find room ${roomId} for GroupCall`);
      return;
    }
    const groupCallId = event.getStateKey();
    const callType = content["m.type"];
    if (!Object.values(_groupCall.GroupCallType).includes(callType)) {
      _logger.logger.warn(`Received invalid group call type ${callType} for room ${roomId}.`);
      return;
    }
    const callIntent = content["m.intent"];
    if (!Object.values(_groupCall.GroupCallIntent).includes(callIntent)) {
      _logger.logger.warn(`Received invalid group call intent ${callType} for room ${roomId}.`);
      return;
    }
    ...
    const groupCall = new _groupCall.GroupCall(this.client, room, callType, isPtt, callIntent, groupCallId, content === null || content === void 0 ? void 0 : content.dataChannelsEnabled, dataChannelOptions);
