...
class MediaHandler extends _typedEventEmitter.TypedEventEmitter {
  ...
  async setAudioInput(deviceId) {
    _logger.logger.info("Setting audio input to", deviceId);
  ...
  async setAudioSettings(opts) {
    _logger.logger.info("Setting audio settings to", opts);
  ...
  async setVideoInput(deviceId) {
    _logger.logger.info("Setting video input to", deviceId);
  ...
  async setMediaInputs(audioInput, videoInput) {
    _logger.logger.log(`mediaHandler setMediaInputs audioInput: ${audioInput} videoInput: ${videoInput}`);
    this.audioInput = audioInput;
    this.videoInput = videoInput;
    await this.updateLocalUsermediaStreams();
  }
  ...
  async updateLocalUsermediaStreams() {
    ...
    for (const stream of this.userMediaStreams) {
      _logger.logger.log(`mediaHandler stopping all tracks for stream ${stream.id}`);
    ...
    for (const call of this.client.callEventHandler.calls.values()) {
      ...
      _logger.logger.log(`mediaHandler updateLocalUsermediaStreams getUserMediaStream call ${call.callId}`);
    ...
    for (const groupCall of this.client.groupCallEventHandler.groupCalls.values()) {
      ...
      _logger.logger.log(`mediaHandler updateLocalUsermediaStreams getUserMediaStream groupCall ${groupCall.groupCallId}`);
  ...
  async getUserMediaStream(audio, video, reusable = true) {
    ...
    if (this.localUserMediaStream) {
      ...
      if (shouldRequestAudio) {
        var _this$localUserMediaS, _this$localUserMediaS2;
        if (this.localUserMediaStream.getAudioTracks().length === 0 || ((_this$localUserMediaS = this.localUserMediaStream.getAudioTracks()[0]) === null || _this$localUserMediaS === void 0 ? void 0 : (_this$localUserMediaS2 = _this$localUserMediaS.getSettings()) === null || _this$localUserMediaS2 === void 0 ? void 0 : _this$localUserMediaS2.deviceId) !== this.audioInput) {
          canReuseStream = false;
        }
      }
      if (shouldRequestVideo) {
        var _this$localUserMediaS3, _this$localUserMediaS4;
        if (this.localUserMediaStream.getVideoTracks().length === 0 || ((_this$localUserMediaS3 = this.localUserMediaStream.getVideoTracks()[0]) === null || _this$localUserMediaS3 === void 0 ? void 0 : (_this$localUserMediaS4 = _this$localUserMediaS3.getSettings()) === null || _this$localUserMediaS4 === void 0 ? void 0 : _this$localUserMediaS4.deviceId) !== this.videoInput) {
          canReuseStream = false;
        }
      }
    } else {
    ...
    if (!canReuseStream) {
      ...
      _logger.logger.log(`mediaHandler getUserMediaStream streamId ${stream.id} shouldRequestAudio ${shouldRequestAudio} shouldRequestVideo ${shouldRequestVideo}`, constraints);
      for (const track of stream.getTracks()) {
      ...
      _logger.logger.log(`mediaHandler clone userMediaStream ${(_this$localUserMediaS5 = this.localUserMediaStream) === null || _this$localUserMediaS5 === void 0 ? void 0 : _this$localUserMediaS5.id} new stream ${stream.id} shouldRequestAudio ${shouldRequestAudio} shouldRequestVideo ${shouldRequestVideo}`);
      if (!shouldRequestAudio) {
  ...
  stopUserMediaStream(mediaStream) {
    _logger.logger.log(`mediaHandler stopUserMediaStream stopping stream ${mediaStream.id}`);
  ...
  async getScreensharingStream(opts = {}, reusable = true) {
    ...
    if (this.screensharingStreams.length === 0) {
      ...
      if (opts.desktopCapturerSourceId) {
        ...
        _logger.logger.debug("Getting screensharing stream using getUserMedia()", opts);
        stream = await navigator.mediaDevices.getUserMedia(screenshareConstraints);
      } else {
        // We are not using Electron
        _logger.logger.debug("Getting screensharing stream using getDisplayMedia()", opts);
  ...
  stopAllStreams() {
    for (const stream of this.userMediaStreams) {
      _logger.logger.log(`mediaHandler stopAllStreams stopping stream ${stream.id}`);
      for (const track of stream.getTracks()) {
        track.stop();
      }
    }
