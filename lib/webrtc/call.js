...
class MatrixCall extends _typedEventEmitter.TypedEventEmitter {
  ...
  async setScreensharingEnabledWithoutMetadataSupport(enabled, opts) {
    _logger.logger.debug(`Call ${this.callId} Set screensharing enabled? ${enabled} using replaceTrack()`);
    if (enabled) {
      try {
        ...
        const sender = (_this$transceivers$ge = this.transceivers.get(getTransceiverKey(_callEventTypes.SDPStreamMetadataPurpose.Usermedia, "video"))) === null || _this$transceivers$ge === void 0 ? void 0 : _this$transceivers$ge.sender;
      ...
      var _this$localUsermediaS3, _this$transceivers$ge2;
      ...
      const sender = (_this$transceivers$ge2 = this.transceivers.get(getTransceiverKey(_callEventTypes.SDPStreamMetadataPurpose.Usermedia, "video"))) === null || _this$transceivers$ge2 === void 0 ? void 0 : _this$transceivers$ge2.sender;
      sender === null || sender === void 0 ? void 0 : sender.replaceTrack(track !== null && track !== void 0 ? track : null);
      this.client.getMediaHandler().stopScreensharingStream(this.localScreensharingStream);
      this.deleteFeedByStream(this.localScreensharingStream);
      return false;
  ...
  async updateLocalUsermediaStream(stream, forceAudio = false, forceVideo = false) {
    ...
    _logger.logger.log(`call ${this.callId} updateLocalUsermediaStream stream ${stream.id} audioEnabled ${audioEnabled} videoEnabled ${videoEnabled}`);
    ...
    for (const track of stream.getTracks()) {
      ...
      const oldSender = (_this$transceivers$ge3 = this.transceivers.get(tKey)) === null || _this$transceivers$ge3 === void 0 ? void 0 : _this$transceivers$ge3.sender;
      let added = false;
      if (oldSender) {
        try {
          _logger.logger.info(`Call ${this.callId} ` + `Replacing track (` + `id="${track.id}", ` + `kind="${track.kind}", ` + `streamId="${stream.id}", ` + `streamPurpose="${callFeed.purpose}"` + `) to peer connection`);
          await oldSender.replaceTrack(track);
          added = true;
        } catch (error) {
          _logger.logger.warn(`replaceTrack failed: adding new transceiver instead`, error);
        }
      }
      if (!added) {
        _logger.logger.info(`Call ${this.callId} ` + `Adding track (` + `id="${track.id}", ` + `kind="${track.kind}", ` + `streamId="${stream.id}", ` + `streamPurpose="${callFeed.purpose}"` + `) to peer connection`);
        const newSender = this.peerConn.addTrack(track, this.localUsermediaStream);
        const newTransciever = this.peerConn.getTransceivers().find(t => t.sender === newSender);
        if (newTransciever) {
          this.transceivers.set(tKey, newTransciever);
        } else {
          _logger.logger.warn("Couldn't find matching transceiver for newly added track!");
        }
      }
    }
  }
  ...
  async setLocalVideoMuted(muted) {
    ...
    _logger.logger.log(`call ${this.callId} setLocalVideoMuted ${muted}`);
   ...
   */
  isLocalVideoMuted() {
    var _this$localUsermediaF3, _this$localUsermediaF4;
    return (_this$localUsermediaF3 = (_this$localUsermediaF4 = this.localUsermediaFeed) === null || _this$localUsermediaF4 === void 0 ? void 0 : _this$localUsermediaF4.isVideoMuted()) !== null && _this$localUsermediaF3 !== void 0 ? _this$localUsermediaF3 : false;
  }
  ...
  async setMicrophoneMuted(muted) {
    ...
    _logger.logger.log(`call ${this.callId} setMicrophoneMuted ${muted}`);
    ...
    (_this$localUsermediaF5 = this.localUsermediaFeed) === null || _this$localUsermediaF5 === void 0 ? void 0 : _this$localUsermediaF5.setAudioVideoMuted(muted, null);
    this.updateMuteStatus();
    await this.sendMetadataUpdate();
    return this.isMicrophoneMuted();
  }
  ...
  isMicrophoneMuted() {
    ...
    return (_this$localUsermediaF6 = (_this$localUsermediaF7 = this.localUsermediaFeed) === null || _this$localUsermediaF7 === void 0 ? void 0 : _this$localUsermediaF7.isAudioMuted()) !== null && _this$localUsermediaF6 !== void 0 ? _this$localUsermediaF6 : false;
  }
  ...
  async sendAnswer() {
    ...
    _logger.logger.info(`Call ${this.callId} Discarding ${discardCount} candidates that will be sent in answer`);
    try {
    ...
    sdp.media.forEach(media => {
      ...
      for (const mod of mods) {
        ...
        if (!codecToPayloadTypeMap.has(mod.codec)) {
          _logger.logger.info(`Ignoring SDP modifications for ${mod.codec} as it's not present.`);
          continue;
        }
        const extraconfig = [];
  ...
  async gotCallFeedsForAnswer(callFeeds) {
    ...
    try {
      ...
      _logger.logger.debug(`Call ${this.callId} Failed to create answer: `, err);
    ...
    try {
      ...
      _logger.logger.debug(`Call ${this.callId} Error setting local description!`, err);
  ...
  async onRemoteIceCandidatesReceived(ev) {
    ...
    if (!candidates) {
      _logger.logger.info(`Call ${this.callId} Ignoring candidates event with no candidates!`);
      return;
    }
    ...
    if (this.opponentPartyId === undefined) {
      ...
      if (fromPartyId) {
        _logger.logger.info(`Call ${this.callId} Buffering ${candidates.length} candidates until we pick an opponent`);
    ...
    if (!this.partyIdMatches(content)) {
      _logger.logger.info(`Call ${this.callId} ` + `Ignoring candidates from party ID ${content.party_id}: ` + `we have chosen party ID ${this.opponentPartyId}`);
      return;
    }
  ...
  async onAnswerReceived(event) {
    ...
    if (this.opponentPartyId !== undefined) {
      _logger.logger.info(`Call ${this.callId} ` + `Ignoring answer from party ID ${content.party_id}: ` + `we already have an answer/reject from ${this.opponentPartyId}`);
      return;
    }
    this.chooseOpponent(event);
    await this.addBufferedIceCandidates();
    ...
    if (sdpStreamMetadata) {
      ...
      _logger.logger.warn(`Call ${this.callId} Did not get any SDPStreamMetadata! Can not send/receive multiple streams`);
    }
    try {
    ...
    if (this.opponentPartyId !== null) {
      try {
        ...
        _logger.logger.warn(`Call ${this.callId} Failed to send select_answer event`, err);
      }
    }
  }
  async onSelectAnswerReceived(event) {
    if (this.direction !== CallDirection.Inbound) {
      _logger.logger.warn(`Call ${this.callId} Got select_answer for an outbound call: ignoring`);
      return;
    }
    const selectedPartyId = event.getContent().selected_party_id;
    if (selectedPartyId === undefined || selectedPartyId === null) {
      _logger.logger.warn(`Call ${this.callId} Got nonsensical select_answer with null/undefined selected_party_id: ignoring`);
      return;
    }
    if (selectedPartyId !== this.ourPartyId) {
      _logger.logger.info(`Call ${this.callId} Got select_answer for party ID ${selectedPartyId}: we are party ID ${this.ourPartyId}.`);
  ...
  async onNegotiateReceived(event) {
    ...
    if (!description || !description.sdp || !description.type) {
      _logger.logger.info(`Call ${this.callId} Ignoring invalid m.call.negotiate event`);
      return;
    }
    ...
    if (this.ignoreOffer) {
      _logger.logger.info(`Call ${this.callId} Ignoring colliding negotiate event because we're impolite`);
      return;
    }
    ...
    if (sdpStreamMetadata) {
      ...
      _logger.logger.warn(`Call ${this.callId} Received negotiation event without SDPStreamMetadata!`);
    }
    try {
      ...
      _logger.logger.warn(`Call ${this.callId} Failed to complete negotiation`, err);
    }
  ...
  async gotLocalOffer() {
    _logger.logger.debug(`Call ${this.callId} Setting local description`);
    if (this.callHasEnded()) {
    ...
    try {
      ...
      _logger.logger.debug(`Call ${this.callId} Failed to create offer: `, err);
    ...
    const discardCount = this.discardDuplicateCandidates();
    _logger.logger.info(`Call ${this.callId} Discarding ${discardCount} candidates that will be sent in offer`);
    try {
      await this.sendVoipEvent(eventType, content);
    } catch (error) {
      _logger.logger.error(`Call ${this.callId} Failed to send invite`, error);
      if (error instanceof _httpApi.MatrixError && error.event) this.client.cancelPendingEvent(error.event);
  ...
  stopAllMedia() {
    _logger.logger.debug(`Call ${this.callId} stopping all media`);
    for (const feed of this.feeds) {
  ...
  async sendCandidateQueue() {
    ...
    _logger.logger.debug(`Call ${this.callId} attempting to send ${candidates.length} candidates`);
    try {
      ...
      if (this.candidateSendTries > 5) {
        _logger.logger.debug(`Call ${this.callId} failed to send candidates on attempt ${this.candidateSendTries}. Giving up on this call.`, error);
      ...
      ++this.candidateSendTries;
      _logger.logger.debug(`Call ${this.callId} failed to send candidates. Retrying in ${delayMs}ms`, error);
      setTimeout(() => {
        this.sendCandidateQueue();
      }, delayMs);
    }
  ...
  chooseOpponent(ev) {
    ...
    _logger.logger.debug(`Call ${this.callId} choosing opponent party ID ${msg.party_id}`);
  ...
  async addBufferedIceCandidates() {
    ...
    if (bufferedCandidates) {
      _logger.logger.info(`Call ${this.callId} Adding ${bufferedCandidates.length} buffered candidates for opponent ${this.opponentPartyId}`);
  ...
  async addIceCandidates(candidates) {
    for (const candidate of candidates) {
      if ((candidate.sdpMid === null || candidate.sdpMid === undefined) && (candidate.sdpMLineIndex === null || candidate.sdpMLineIndex === undefined)) {
        _logger.logger.debug(`Call ${this.callId} got remote ICE end-of-candidates`);
      } else {
      ...
      try {
        ...
        if (!this.ignoreOffer) {
          _logger.logger.info(`Call ${this.callId} failed to add remote ICE candidate`, err);
        }
      }
    }
  }
