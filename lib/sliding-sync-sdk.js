...
class ExtensionToDevice {
  ...
  async onResponse(data) {
    ...
    (_data$events = data.events) === null || _data$events === void 0 ? void 0 : _data$events.map(this.client.getEventMapper()).map(toDeviceEvent => {
...
class SlidingSyncSdk {
  ...
  constructor(slidingSync, client, opts = {}) {
    ...
    const extensions = [new ExtensionToDevice(this.client), new ExtensionAccountData(this.client), new ExtensionTyping(this.client), new ExtensionReceipts(this.client)];
    if (this.opts.crypto) {
  ...
  async processRoomData(client, room, roomData) {
    ...
    const processRoomEvent = async e => {
      ...
      if (e.isState() && e.getType() == _event.EventType.RoomEncryption && this.opts.crypto) {
