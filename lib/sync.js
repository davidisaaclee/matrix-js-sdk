...
class SyncApi {
  ...
  */
   ...
   */
  stop() {
    var _global$window2, _global$window2$remov, _this$currentSyncRequ;
  ...
  async processSyncResponse(syncEventData, data) {
    ...
    await utils.promiseMapSeries(joinRooms, async joinObj => {
      ...
      if (joinObj.unread_notifications) {
        room.setUnreadNotificationCount(_room.NotificationCountType.Total, joinObj.unread_notifications.notification_count);
      ...
      if (joinObj.isBrandNewRoom) {
        ...
        if (limited) {
          room.resetLiveTimeline(joinObj.timeline.prev_batch, this.opts.canResetEntireTimeline(room.roomId) ? null : syncEventData.oldSyncToken);
  ...
  pokeKeepAlive(connDidFail = false) {
    ...
    this.client.http.request(undefined,
    // callback
    _httpApi.Method.Get, "/_matrix/client/versions", undefined,
    // queryParams
    undefined,
    // data
    {
