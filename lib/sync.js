...
class SyncApi {
  ...
  constructor(client, opts = {}) {
    ...
    (0, _defineProperty2.default)(this, "checkLazyLoadStatus", async () => {
      ...
      if (this.opts.lazyLoadMembers) {
        var _this$opts$crypto;
        (_this$opts$crypto = this.opts.crypto) === null || _this$opts$crypto === void 0 ? void 0 : _this$opts$crypto.enableLazyLoading();
      }
      try {
  ...
  async processSyncResponse(syncEventData, data) {
    var _data$presence, _data$account_data, _data$to_device;
    const client = this.client;
    ...
    if (Array.isArray((_data$to_device = data.to_device) === null || _data$to_device === void 0 ? void 0 : _data$to_device.events) && data.to_device.events.length > 0) {
      ...
      data.to_device.events.filter(eventJSON => {
        var _eventJSON$content;
        if (eventJSON.type === _event.EventType.RoomMessageEncrypted && !["m.olm.v1.curve25519-aes-sha2"].includes((_eventJSON$content = eventJSON.content) === null || _eventJSON$content === void 0 ? void 0 : _eventJSON$content.algorithm)) {
      ...
      if (client.isCryptoEnabled()) {
        ...
        for (const parked of parkedHistory) {
          if (parked.senderId === inviter) {
            await client.crypto.olmDevice.addInboundGroupSession(room.roomId, parked.senderKey, parked.forwardingCurve25519KeyChain, parked.sessionId, parked.sessionKey, parked.keysClaimed, true, {
              sharedHistory: true,
              untrusted: true
            });
          }
    ...
    await utils.promiseMapSeries(joinRooms, async joinObj => {
      ...
      if (joinObj.isBrandNewRoom) {
        ...
        if (limited) {
          ...
          room.resetLiveTimeline(joinObj.timeline.prev_batch, this.opts.canResetEntireTimeline(room.roomId) ? null : (_syncEventData$oldSyn = syncEventData.oldSyncToken) !== null && _syncEventData$oldSyn !== void 0 ? _syncEventData$oldSyn : null);
    ...
    if (this.opts.crypto && (data.device_unused_fallback_key_types || data["org.matrix.msc2732.device_unused_fallback_key_types"])) {
      ...
      this.opts.crypto.setNeedsNewFallback(Array.isArray(unusedFallbackKeys) && !unusedFallbackKeys.includes("signed_curve25519"));
    }
  }
