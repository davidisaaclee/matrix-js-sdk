...
class MatrixClient extends _typedEventEmitter.TypedEventEmitter {
  ...
  constructor(opts) {
    ...
    this.on(_event.MatrixEventEvent.Decrypted, event => {
      ...
      if (!room) return;
      const currentCount = room.getUnreadNotificationCount(_matrix.NotificationCountType.Highlight);
  ...
  async getEventTimeline(timelineSet, eventId) {
    ...
    if (this.supportsExperimentalThreads()) {
      ...
      if (_thread.Thread.hasServerSideSupport && timelineSet.thread) {
        ...
        let nextBatch = thread.liveTimeline.getPaginationToken(_eventTimeline.Direction.Backward);
  ...
  getVersions() {
    ...
    this.serverVersionsPromise = this.http.request(undefined,
    // callback
  ...
  refreshToken(refreshToken, options = {}) {
    return this.http.authedRequest(undefined, _httpApi.Method.Post, "/refresh", undefined, {
      ...
      prefix: options.forcePrefixV1 ? _httpApi.PREFIX_V1 : _httpApi.PREFIX_V3,
