...
class MatrixClient extends _typedEventEmitter.TypedEventEmitter {
    ...
    const deleteRustSdkStore = async () => {
      ...
      for (const dbname of [`${_constants.RUST_SDK_STORE_PREFIX}::matrix-sdk-crypto`, `${_constants.RUST_SDK_STORE_PREFIX}::matrix-sdk-crypto-meta`]) {
        const prom = new Promise((resolve, reject) => {
          ...
          req.onerror = e => {
            _logger.logger.error(`Failed to remove IndexedDB instance ${dbname}: ${e}`);
  ...
  redactEvent(roomId, threadId, eventId, txnId, opts) {
    var _eventId, _opts, _opts2, _opts3, _opts4;
    ...
    const reason = (_opts = opts) === null || _opts === void 0 ? void 0 : _opts.reason;
    if ((_opts2 = opts) !== null && _opts2 !== void 0 && _opts2.with_relations && this.canSupport.get(_feature.Feature.RelationBasedRedactions) === _feature.ServerSupport.Unsupported) {
    ...
    const withRelations = (_opts3 = opts) !== null && _opts3 !== void 0 && _opts3.with_relations ? {
      [this.canSupport.get(_feature.Feature.RelationBasedRedactions) === _feature.ServerSupport.Stable ? _event2.MSC3912_RELATION_BASED_REDACTIONS_PROP.stable : _event2.MSC3912_RELATION_BASED_REDACTIONS_PROP.unstable]: (_opts4 = opts) === null || _opts4 === void 0 ? void 0 : _opts4.with_relations
    } : {};
    return this.sendCompleteEvent(roomId, threadId, {
  ...
  async sendReceipt(event, receiptType, body, unthreaded = false) {
    ...
    const supportsThreadRR = this.canSupport.get(_feature.Feature.ThreadUnreadNotifications) !== _feature.ServerSupport.Unsupported;
...
function fixNotificationCountOnDecryption(cli, event) {
  ...
  const currentCount = room.getUnreadCountForEventContext(_room.NotificationCountType.Highlight, event);
  ...
  if (oldHighlight !== newHighlight || currentCount > 0) {
    ...
    if (isThreadEvent) {
      ...
      hasReadEvent = thread ? thread.hasUserReadEvent(cli.getUserId(), event.getId()) :
    ...
    if (!hasReadEvent) {
      ...
      const totalCount = (_ref3 = isThreadEvent ? room.getThreadUnreadNotificationCount(event.threadRootId, _room.NotificationCountType.Total) : room.getRoomUnreadNotificationCount(_room.NotificationCountType.Total)) !== null && _ref3 !== void 0 ? _ref3 : 0;
