...
class MegolmDecryption extends _base.DecryptionAlgorithm {
  ...
  async sendSharedHistoryInboundSessions(devicesByUser) {
    ...
    _logger.logger.log(`Sharing history in ${this.roomId} with users ${Object.keys(devicesByUser)}`, sharedHistorySessions.map(([senderKey, sessionId]) => `${senderKey}|${sessionId}`));
