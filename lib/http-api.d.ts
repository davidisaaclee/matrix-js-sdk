...
type RequestProps = "method" | "withCredentials" | "json" | "headers" | "qs" | "body" | "qsStringifyOptions" | "useQuerystring" | "timeout";
...
type ResponseType<T, O extends IRequestOpts<T> | void = void> = O extends {
    bodyParser: (body: string) => T;
} ? T : O extends {
    json: false;
} ? string : T;
...
export type UploadContentResponseType<O extends IUploadOpts> = O extends undefined ? string : O extends {
    rawResponse: true;
} ? string : O extends {
    onlyContentUri: true;
} ? string : O extends {
